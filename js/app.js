...
function showAnalysisTab(tab) {
  const tabs = ['overview','locations','factions','npcs','monsters','report'];
  tabs.forEach(t => {
    document.getElementById('analysis' + t.charAt(0).toUpperCase() + t.slice(1)).classList.remove('active');
  });
  document.querySelectorAll('.analysis-tab').forEach(el => el.classList.remove('active'));
  document.getElementById('analysis' + tab.charAt(0).toUpperCase() + tab.slice(1)).classList.add('active');
